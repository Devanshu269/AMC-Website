window.onload = function() {
    ...
    $(document).ready(function() {
        ...
        var table = $('#user_data').DataTable({
            ...
            columns: [
            {  title: 'USN', data: USN},
            {  title: 'Branch', data: Program },
            {  title: 'Section', data: Student_Section},
            {  title: 'SEM', data: sem},
        ...
        function avgAge() {
            let columnData = $('#user_data').DataTable().column(3,{search:'applied'}).data().toArray();
            return Math.round(columnData.reduce((sum, item) => sum+=item)/columnData.length);
          };
         
          //Append <tfoot>
          $('#user_data').append(`<tfoot><tr><td colspan="3">Average age: <span id="avgage">${avgAge()}</span></td></tr></tfoot>`);
